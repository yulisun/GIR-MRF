function [Z, delt,RelDiff] = StructureConsistencyImageRegressionPCG(Y,L,W,opt)
...
IZT = (eye(N)-W)';
A = mu1*eye(N)+4*L+IZ_IZT;
sparse_level = round(N*0.3);
Z = Y;
...
for i=1:iter
    ...
    Z = Zupdate_PCG(A,(mu1*(delt+Y)+mu2*E*IZT-R1-R2*IZT));% PCG
